...
import { categoryModalKey } from "./DialogCategory";
...
const routeBase = "admin.news-categories";
...
function TableCategory() {
    ...
    const {
        props: { newsCategory },
    } = usePage<NewsCategoryIndexProps>();
    ...
    const handleEdit = (id?: string) => {
        if (!id) return;
        const model = newsCategory.find((category) => category.id === id);
        if (!model) return;
        console.log(`Ingresa a editar ${model}`);
        toggleModal(categoryModalKey, true, id, model);
    };
            ...
            <CustomTable
                ...
                renderRow={(category) => (
                    <>
                        ...
                        <TableCell>
                            <p>{category.description ?? "Descripción"}</p>
                        </TableCell>
