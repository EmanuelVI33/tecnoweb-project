...
import { subscriptionModalKey } from "./DialogSubscription";
import { Subscription } from '../../Models/subscription';

const routeBase = 'subscriptions';

const headers = ["Nombre", "Descripción", "Price", "Duración", "Acciones"];

function TableSubscription() {
    const { toggleModal } = useModalStore();
    const { props: { subscriptions } } = usePage<SubscriptionIndexProps>();
    ...
    const handleEdit = (id?: string) => {
        if (!id) return;
        const model = subscriptions.find((subscription) => subscription.id === id);
        if (!model) return;
        console.log(`Ingresa a editar ${model}`);
        toggleModal(subscriptionModalKey, true, id, model);
    };
    ...
    return (
        <>
            <CustomTable
                caption="Lista de Subscripciones"
                headers={headers}
                data={subscriptions}
                renderRow={(Subscription) => (
                    <>
                        ...
                        <TableCell>
                            <p>{Subscription.description ?? 'Descripción'}</p>
                        </TableCell>
                        <TableCell>
                            <p>{Subscription.price}</p>
                        </TableCell>
                        <TableCell>
                            <p>{Subscription.duration} días</p>
                        </TableCell>
                        <TableCell className="text-end flex justify-around ">
                            {/* <Button onClick={() => handleOpen(Subscription.id)}>
            ...
            <ConfirmationModal
                ...
                description="¿Estás seguro de que deseas eliminar esta Categoría de Noticias? Esta acción no se puede deshacer."
            />
        </>
    );
}
