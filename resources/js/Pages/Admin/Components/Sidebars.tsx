...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 h-full w-9/12 md:w-1/3 lg:w-1/6 bg-gray-200 dark:bg-gray-700 shadow-md transition-transform duration-300 ${isOpen ? 'transform translate-x-0' : 'transform -translate-x-full'}`}>
                  ...
                  <ul className={`pl-6 transition-max-height duration-300 ease-in-out overflow-hidden ${menuOpen[option.id] ? 'max-h-screen' : 'max-h-0'}`}>
                    {option.subItems.map((subItem, index) => (
                      <li className="mb-2" key={index}>
                        <Link href={route(subItem.url)}  className="flex items-center text-gray-900 dark:text-gray-100 p-2 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition duration-200">
                          {subItem.label}
                        </Link>
                      </li>
                    ))}
