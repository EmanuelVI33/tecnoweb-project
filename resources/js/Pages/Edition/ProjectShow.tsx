...
function ProjectShow({ project, success, error }: ProjectShowPageProps) {
    ...
    const { post, wasSuccessful, data, errors } = useForm<{
        elements: Element[];
        project_id: string;
    }>({
        elements: [],
    ...
    useEffect(() => {
        const elements = ElementMapper.fromElementResponseToEntity(
            project.elements
        );
    ...
    const handleGenerate = async () => {
        ...
        post(route("elements.generate", project.id), {
            onError: (errors) => {
                console.log(errors);
            },
        });
            ...
            <div className="grid grid-rows-4 h-full">
                <div className="row-span-3 grid grid-cols-2 h-full">
                    <Tabs
                        ...
                        className="bg-gray-200 dark:bg-gray-900 text-gray-900 dark:text-gray-100 px-4 py-1"
                        color=""
                    >
                        <TabsList className="">
                ...
                <div className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-100 row-span-1 ">
                    <div className="flex justify-between mx-5">
                        <h1 className="text-2xl tracking-tight hover:tracking-wid font-bold">
                            Guion
                        </h1>
